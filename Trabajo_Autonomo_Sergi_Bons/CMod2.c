...
double bias = 0.0;
int red, blue, green;
void apply_filter(int* image,int x, int y,int nFilters)
{
		red = 0;
		blue = 0;
		green = 0;
...
void truncate(int * result, int x, int y, int nFilters)
{
...
int main(int argc, char* argv[])
{
	...
	for (int y = 0; y < h; y++)
            for (int x = 0; x < w; x++)
            {
		apply_filter(image,x,y,nFilters);
                //truncate values smaller than zero and larger than 255
                truncate(result,x, y, nFilters);
            }
